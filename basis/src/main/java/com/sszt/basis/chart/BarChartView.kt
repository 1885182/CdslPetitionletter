...
class BarChartView : View {
    ...
    private var isShowBarTopText = true
    private var strFormat = "%.0f"
    ...
    private fun init(attrs: AttributeSet?) {
        attrs?.apply {
            ...
            isShowBarTopText = ta.getBoolean(R.styleable.BarChartView_isShowBarTopText, true)
            strFormat = ta.getString(R.styleable.BarChartView_strFormat) ?: strFormat
    ...
    private fun drawFormContent(canvas: Canvas) {
        ...
        for (i in mData.indices) {
            ...
            try {
                var dataH = mData[i].data * mFormYBarSpace.toBigDecimal()
                barHeight = (dataH / mYSpaceSize).toFloat()
            } catch (e: Exception) {
                barHeight = 0f
            }
...
//                str = mData[i].data.toString()
    ...
    /**
     * @param format   String.format("%.nf",d);-n-表示保留N位 ;d--表示原数值
     */

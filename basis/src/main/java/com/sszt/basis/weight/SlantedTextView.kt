...
import com.sszt.basis.weight.SlantedTextView
...
class SlantedTextView : View {
    ...
    var maxTextLength: Int = Int.MAX_VALUE
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        maxTextLength =
            array.getInt(R.styleable.SlantedTextView_maxTextLength, Int.MAX_VALUE)
        mSlantedBackgroundColor = array.getColor(
            R.styleable.SlantedTextView_slantedBackgroundColor,
            mSlantedBackgroundColor
        )

        isSub = array.getBoolean(R.styleable.SlantedTextView_isSub, true)
    ...
    private fun subText() {
        ...
        if (maxTextLength != Int.MAX_VALUE && text?.length ?: 0 > maxTextLength) {
            text = text?.substring(0, maxTextLength)
            return
        }

        if (isSub) {
            val measureText = mTextPaint!!.measureText(text, 0, text!!.length)
            if (measureText > width) {
                text = text?.substring(0, (width.div(measureText.div(text!!.length))).toInt())
            }
        }
    }

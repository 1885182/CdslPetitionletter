...
package com.ren.newalbumchoose.video;
...
public class UniversalVideoView extends SurfaceView
    ...
    public UniversalVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mFitXY = a.getBoolean(R.styleable.UniversalVideoView_uvv_fitXY, true);
        mAutoRotation = a.getBoolean(R.styleable.UniversalVideoView_uvv_autoRotation, true);
        a.recycle();
        initVideoView();
    }
    ...
    public void setVideoPath(String path) {
        setVideoURI(Uri.parse(setCache(path)));
    }
    ...
    private HttpProxyCacheServer httpCache;

    /**
     * 设置视频缓存
     * 有缓存用缓存、没缓存第一次用原始的、并且缓存 之后用缓存的
     *
     * @param videoUrl
     * @return
     */
    private String setCache(String videoUrl) {
        //
        httpCache = new HttpProxyCacheServer.Builder(getContext())
                .maxCacheSize(1024 * 1024 * 1024)//最大缓存个数
                .cacheDirectory(new File(getContext().getExternalFilesDir(Environment.DIRECTORY_MOVIES).getAbsolutePath()))//缓存路径
                .build();
        return httpCache.getProxyUrl(videoUrl);

    }

    public void resumeOrPause() {
        if (mMediaController != null) {
            mMediaController.resumeOrPause();
        }
    }
...
//                    }
     ...
     *
     * @param l The callback that will be run
     *           http://103.215.32.6:8080/AIPbHelp/aidj_video/kcenter/scene/新建党的基层组织.mp4
     */
    ...
    SurfaceHolder.Callback mSHCallback = new SurfaceHolder.Callback() {
        public void surfaceChanged(SurfaceHolder holder, int format,
            ...
            mSurfaceWidth = w;
            mSurfaceHeight = h;
            Log.d(TAG, String.format("onVideoSizeChanged  Surface  width=%d,height=%d", mSurfaceWidth, mSurfaceHeight));
